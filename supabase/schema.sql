...
-- TEMP DIAGNOSTIC: force a failure that dumps the live create_booking
-- definition into the CI error annotation, so it can be read from outside
-- the sandbox. Removed again right after this run.
do $$
begin
  raise exception 'DIAG_FUNC_DEF: %', pg_get_functiondef('public.create_booking(text,text,date,text,text)'::regprocedure);
end $$;
